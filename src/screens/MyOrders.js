...
const MyOrders = ({navigation}) => {
  ...
  const onChangeSearch = query => {
    setdoctorsfilter(doctors.filter(i => i.patient_name.includes(query)));
    setlabsfilter(labs.filter(i => i.patient_name.includes(query)));
    setDialogdoc(null);
    setDialoglab(null);
  };
  ...
  useEffect(async () => {
    ...
    setallOrders(datas.data().schedule);
    datas.data().schedule.map((source, index) => {
      if (source.type == 'labtest') {
        ...
        if (labs.some(mem => mem.timestamp === source.timestamp)) {
          ...
          setlabs(prev => [...prev, {...source, ind: index}]);
          setlabsfilter(prev => [...prev, {...source, ind: index}]);
        ...
        if (doctors.some(mem => mem.timestamp === source.timestamp)) {
          ...
          setdoctors(prev => [...prev, {...source, ind: index}]);
          setdoctorsfilter(prev => [...prev, {...source, ind: index}]);
          setMemlist(prevs => [...prevs, source.patient_name]);
          console.log('already exists  patient ', source.patient_name);
        }
      }
    ...
    const datass = await firestore().collection('clinics').get();
    ...
    const familydatas = await firestore()
      .collection('user')
      .doc(user.uid)
      .collection('family')
      .get();
                ...
                <View style={[styles.spacebetween, {marginTop: 8}]}>
                  <View style={styles.left}>
                    <Text style={styles.usertext}>
                      {clinicname(source.clinic_id)}
                    </Text>
                    <Text style={styles.subText}>
                      {clinicaddress(source.clinic_id)}
                    </Text>
                  </View>
                    ...
                    <View
                      ...
                      <TouchableOpacity
                        onPress={() => {
                          ...
                          navigation.navigate('Bookcheckupupdate', {
                            cart: {data: allOrders, ordered: true},
                            index: source.ind,
                          });
                        }}
                        style={styles.butt}>
                  ...
                  <View
                    ...
                    <View
                      onPress={() => {
                        ...
                        newArray[index] = {
                          ...source,
                          collection_option: {clinic: false, home: true},
                        };
                        setlabs(newArray);
                      }}
                      style={[styles.radioBorder, {}]}>
            ...
            <TouchableOpacity
              ...
              <View style={{flexDirection: 'row', alignItems: 'center'}}>
                <Image
                  ...
                  source={links.user}
                />
                <Text style={[styles.time, {color: '#373737'}]}>All</Text>
              </View>
            </TouchableOpacity>
