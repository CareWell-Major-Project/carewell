...
export default function Clinicdetails({navigation, route}) {
  ...
  async function setcompletelabs() {
    // setpackages([]);
    // console.log(details.clinicDocId, 'lab doc id');
    const temp = [];
    await firestore()
      .collection('clinics')
      .doc(details.clinicDocId)
      .collection('labtests')
      ...
      .then(querySnapshot => {
        querySnapshot.forEach(documentSnapshot2 => {
          console.log(documentSnapshot2.data());
          temp.push(documentSnapshot2.data());
          setlastlab(documentSnapshot2.id);
        });
      });
  ...
  async function retrieveMore() {
    ...
    await firestore()
      ...
      .then(querySnapshot => {
        querySnapshot.forEach(documentSnapshot2 => {
          console.log(documentSnapshot2.data());
          setlabs(prevstate => [...prevstate, documentSnapshot2.data()]);
          setlastlab(documentSnapshot2.id);
        });
      });
    // setpackages(...packages, temp);
                                          ...
                                          {sourcee.id == details.id ? (
                                            <View
                                              ...
                                              <Text style={styles.time}>
                                                Timing - {sourcee.timing.intime}
                                                :00am-{sourcee.timing.outime}
                                                :00am
                                              </Text>
                                              <Text style={styles.time}>
                                                Days -
                        ...
                        <View style={[styles.spacebetween, {marginTop: 13}]}>
                          ...
                          <TouchableOpacity
                            ...
                            onPress={() => {
                              ...
                              navigation.navigate('BooklabCalender', {
                                source: {
                                  test_name: source.package_name,
                                  lab_name: source.lab_name,
                                  clinic_name: details.name,
