...
import merchantInfo from '../services/config';
import {getChecksumString, calculateChecksum} from '../services/checksum';
...
export default function Cart(props) {
  ...
  const getuserdetail = uid => {
    const subscriber = firestore()
      ...
      .onSnapshot(documentSnapshot => {
        ...
        // setFamilymem([...familymem, {name: documentSnapshot.data().Info.name}]);
      });
  ...
  async function test() {
    ...
    const userData = await firestore().collection('user').doc(user.uid).get();
    let famArray = [{name: userData.data().Info.name}];
  ...
  function onSuccessfullPay(amount, data) {
    CartItems.map((source, index) => {
      source?.type == 'checkup'
        ...
        : firestore()
            .doc('appointment/' + source.lab_id.toString())
            .set(
              {
                bookedslots: firestore.FieldValue.arrayUnion(source.timestamp),
              },
      ...
      firestore()
        ...
        .update({
          schedule: firestore.FieldValue.arrayUnion({
            ...
            staff: {
              referral: {
                OPD_incentive: totalstaffrefdic,
                code: code,
              },
            },
          }),
    ...
    updateAppointment_schedule(CartItems);
  ...
  const addPaymentData = (amount, data) => {
    firestore()
      .collection('payments')
      .add({
        ...data,
  ...
  updateAppointment_schedule = cartData => {
    cartData.map((source, index) => {
      source?.type == 'checkup'
        ? firestore()
            .collection('appointment_schedule')
            .doc(source.doctor.phone_no)
            .set(
              {
  ...
  const createOrderId = async amount => {
    ...
    await fetch('https://api.razorpay.com/v1/orders', {
      ...
      headers: {
        Authorization:
          'Basic cnpwX2xpdmVfU09venBIVTBQZ1NQT2o6RjVWU1RyaEcyNFlPMWo2ZjRuWXF5anlj',
      ...
      .then(order => {
        ...
        // return order.id;
        orderId = order.id;
        checkOut(amount, order.id);
      });
    // return orderId;
  };

  const checkOut = async (amount, orderId) => {
    console.log(orderId, 'orderid');
    var options = {
      description: 'CareWell app',
      image: 'https://i.imgur.com/3g7nmJC.png',
      currency: 'INR',
      // key: 'rzp_live_SOozpHU0PgSPOj',
    ...
    // /*Zaakpay*/
    // // console.log(merchantInfo.transactApi);
    // let orderId = 'L13sijeoinfiknr',
    //   buyerEmail = 'itsyashakaalone@gmail.com',
    //   currency = 'INR';
    // var checksumstring = await getChecksumString({
    //   amount,
    //   buyerEmail,
    //   currency,
    ...
    // Linking.openURL(
    //   `https://zaakstaging.zaakpay.com/api/paymentTransact/V8?amount=${amount}&buyerEmail=${buyerEmail}&currency=INR&merchantIdentifier=d22b6680ce804b1a81cdccb69a1285f1&orderId=${orderId}&checksum=${calculatedchecksum}`,
    // ).then(k => {
    //   console.log(k);
    // });
  };
                    ...
                    {source?.type == 'checkup' ? (
                      <View>
                        <Text style={styles.name}>
                          {source?.type != null ? source?.doctor.name : ''}{' '}
                        </Text>
          ...
          <View style={styles.modalViewMem}>
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    setSelectedName({name: source.name, id});
                  }}
                  style={[
                    styles.spacebetween,
                    {
                      borderBottomWidth: 1,
                      backgroundColor:
                        selectedName.id == id ? '#835CB980' : null,
                      paddingVertical: 5,
                    },
                  ]}>
                  <View style={{flexDirection: 'row', alignItems: 'center'}}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                activefamily(selectedName.name, dialogmem);
                // setselectedoption(-1);
                // setToggleCheckBox(false);
                setDialogmem(null);
              }}>
