...
export default function Bookcheckupupdate({navigation, route}) {
  ...
  const [schedarray, setschedarray] = React.useState([]);
  ...
  useEffect(() => {
    ...
    console.log('cart data ', cart.data[index]);
    settemp(cart.data[index]);
    console.log('checkup');
    ...
    cart.data[index]?.type == 'checkup'
      ? firestore()
          .collection('appointment_schedule')
          .doc(cart.data[index].doctor.phone_no)
          .get()
          .then(da => {
            setschedarray(da.data().appointments);
            da.data().appointments.map((source, ind) => {
              if (source.date.timestamp == cart.data[index].date.timestamp) {
                setindexed(ind);
              }
            });
          })
      : firestore()
          .collection('appointment_schedule')
          .doc(cart.data[index].clinic.id.toString())
          .get()
          .then(da => {
            setschedarray(da.data().appointments);
            da.data().appointments.map((source, ind) => {
  ...
  function addtocart() {
    ...
    if (cart?.ordered) {
      ...
      console.log('cart data ', cart.data[index]);
      let newArray = [...schedarray];
      newArray[indexed] = selectedslotdetails;
      setschedarray(newArray);
  ...
  const changeschedule = newArray => {
    temp?.type == 'checkup'
      ? firestore()
          .collection('appointment_schedule')
          .doc(temp.doctor.phone_no)
          .set({
